...
public class ScotlandYard {
    ...
    public static DirectedGraph<Integer> getGraph() throws FileNotFoundException {
        ...
        Scanner in = new Scanner(new File("Aufgabe3/shortestPath/ScotlandYard_Kanten.txt"));

        while (in.hasNextInt()) {
            int u = in.nextInt();
            int v = in.nextInt();
            double w = switch (in.next()) {
                case "UBahn" -> 5.0;
                case "Taxi" -> 2.0;
                case "Bus" -> 3.0;
                default -> 0.0;
            };
            if (sy_graph.containsVertex(u) && sy_graph.containsVertex(v)) {
                if (sy_graph.containsEdge(u, v)) {
                    if (sy_graph.getWeight(u, v) < w) {
                        continue;
                    }
                }
            }
            sy_graph.addEdge(u, v, w);
            sy_graph.addEdge(v, u, w);
        }
