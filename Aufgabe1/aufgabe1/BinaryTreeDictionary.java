...
public class BinaryTreeDictionary<K extends Comparable<? super  K>, V> implements Dictionary<K, V> {
    ...
    private Node<K,V> insertR(K key, V value, Node<K,V> p) {
        ...
        else { // Schlüssel bereits vorhanden:
            oldValue = p.value;
            p.value = value;
        }
        p = balance(p);
...
// p muss gelöscht werden und hat ein oder kein Kind:
            oldValue = p.value;
            p = (p.left != null) ? p.left : p.right;
        } else {
// p muss gelöscht werden und hat zwei Kinder:
            MinEntry<K,V> min = new MinEntry<>(); p.right = getRemMinR(p.right, min);
            oldValue = p.value;
            p.key = min.key;
            p.value = min.value;
        }
